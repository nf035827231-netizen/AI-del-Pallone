...
async function settleOne(row){const id=String(row.event_id||'').replace(/^sofascore-/,'');if(!/^\d+$/.test(id))return null;try{const r=await fetch(`https://www.sofascore.com/api/v1/event/${encodeURIComponent(id)}`,{headers:{Accept:'application/json','Origin':'https://www.sofascore.com','Referer':'https://www.sofascore.com/'},cache:'no-store'});if(!r.ok)return null;const m=await r.json();const ev=m?.event||m;const finished=String(ev?.status?.type||ev?.status?.description||'').toLowerCase().includes('finish')||Number(ev?.status?.code)===100;if(!finished)return null;const hg=Number(ev?.homeScore?.current??ev?.homeScore?.normaltime??ev?.homeScore?.display),ag=Number(ev?.awayScore?.current??ev?.awayScore?.normaltime??ev?.awayScore?.display);if(!Number.isFinite(hg)||!Number.isFinite(ag))return null;return evaluateMarket(row.market,hg,ag);}catch{return null;}}
